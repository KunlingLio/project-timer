...
export function init(): vscode.Disposable {
    ...
    const changeListener = vscode.workspace.onDidChangeWorkspaceFolders(() => {
        logger.warn("[Refresher] Workspace folders changed, refreshing ALL!");
        refresh();
    });
...
function periodicRefresh() {
    logger.log(`[Refresher] Periodic refresh triggered after ${lastInterval / 1000} seconds`);
    refresh();
    ...
    logger.log(`[Refresher] Next periodic refresh scheduled in ${lastInterval / 1000} seconds`);
}
...
export function refresh() {
    callbacks.forEach(callback => {
        try {
            ...
            logger.error(`[Refresher] Error in refresh callback: ${e}`);
        }
    });
}
